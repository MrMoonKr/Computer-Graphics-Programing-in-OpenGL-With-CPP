...
std::string g_Title = "4-4. 행렬스택 사용하기";
std::string g_FPS   = "";
...
float       g_PyramidX = 0.0f ;
float       g_PyramidY = 0.0f ;
float       g_PyramidZ = 0.0f ;
...
void setupVertices(void) 
{
    ...
    glGenVertexArrays( numVAOs, g_VAO );  // creates VAO and returns the integer ID
...
void app_display( GLFWwindow* window )
{
    ...
    mvStack.push( mvStack.top() );
    mvStack.top() *= glm::rotate( glm::mat4(1.0f), currentTime, glm::vec3( 0.0f, 0.0f, 1.0f ) );

    glUniformMatrix4fv( g_ViewMatrixLocation, 1, GL_FALSE, glm::value_ptr( mvStack.top() ) );
    glBindBuffer( GL_ARRAY_BUFFER, g_VBO[1] );
    ...
    mvStack.push( mvStack.top() );
    mvStack.top() *= glm::translate( glm::mat4(1.0f), glm::vec3( sin( currentTime ) * 4.0f, 0.0f, cos( currentTime ) * 4.0f ) );
    mvStack.push( mvStack.top() );
    mvStack.top() *= glm::rotate( glm::mat4(1.0f), -currentTime, glm::vec3( 0.0f, 1.0f, 0.0f ) );
    ...
    // draw the cube mom
    mvStack.push( mvStack.top() );
    mvStack.top() *= glm::translate( glm::mat4(1.0f), glm::vec3( sin( currentTime ) * 2.0f, 0.0f, cos( currentTime ) * 2.0f ) );
    mvStack.push( mvStack.top() );
    mvStack.top() *= glm::rotate( glm::mat4(1.0f), currentTime, glm::vec3( 0.0f, 0.0f, 1.0f ) );
