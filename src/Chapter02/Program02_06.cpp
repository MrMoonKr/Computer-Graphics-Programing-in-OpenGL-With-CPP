...
{
    g_Location += g_MoveSpeed * deltaTime;
    if ( g_Location > 1.0f )
    {
        g_MoveSpeed = -0.5f;
    }
    else if ( g_Location < -1.0f )
    {
        g_MoveSpeed = 0.5f;
    }
}
...
void app_display()
{
    ...
    GLuint offsetLoc = glGetUniformLocation( g_RenderingProgram, "offset" );
    glProgramUniform1f( g_RenderingProgram, offsetLoc, g_Location );
